...
public class WebSocketTransport extends AbsWebSocketTransport {
  ...
  public void connect(Listener listener) {
    Logging.INSTANCE.debug(TAG, "connect()");
    mListener = listener;
  ...
  private boolean scheduleReconnect() {
    ...
    Logging.INSTANCE.debug(TAG, "scheduleReconnect() ");
    mHandler.postDelayed(
        () -> {
          if (mClosed) {
            return;
          }
          Logging.INSTANCE.info(TAG, "doing reconnect job, retryCount: " + mRetryStrategy.retryCount);
  ...
  public void close() {
    ...
    mClosed = true;
    Logging.INSTANCE.debug(TAG, "close()");
  ...
  private class ProtooWebSocketListener extends WebSocketListener {
    ...
    public void onOpen(@NotNull WebSocket webSocket, @NotNull Response response) {
      ...
      Logging.INSTANCE.debug(TAG, "onOpen() ");
    ...
    public void onClosed(@NotNull WebSocket webSocket, int code, @NotNull String reason) {
      Logging.INSTANCE.info(TAG, "onClosed()");
      if (mClosed) {
        return;
      }
      mClosed = true;
    ...
    public void onFailure(
      ...
      if (scheduleReconnect()) {
        ...
        Logging.INSTANCE.error(TAG, "give up reconnect. notify closed");
        mClosed = true;
        if (mListener != null) {
          mListener.onClose();
        }
    ...
    public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
      Logging.INSTANCE.debug(TAG, "onMessage()");
      if (mClosed) {
        return;
      }
  ...
  private OkHttpClient getUnsafeOkHttpClient() {
    try {
      ...
      HttpLoggingInterceptor httpLoggingInterceptor =
          new HttpLoggingInterceptor(s -> Logging.INSTANCE.debug(TAG, s));
