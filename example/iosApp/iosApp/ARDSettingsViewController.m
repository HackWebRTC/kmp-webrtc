    ...
    switch (indexPath.section) {
        case ARDSettingsSectionVideoResolution:
            [self tableView:tableView
                disSelectVideoResolutionAtIndex:indexPath];
            break;

        case ARDSettingsSectionVideoCodec:
            [self tableView:tableView
                didSelectVideoCodecCellAtIndexPath:indexPath];
            break;
    }
}
...
videoResolutionTableViewCellForTableView:(UITableView*)tableView
    ...
    NSString* dequeueIdentifier =
        @"ARDSettingsVideoResolutionViewCellIdentifier";
...
- (UITableViewCell*)bitrateTableViewCellForTableView:(UITableView*)tableView
    ...
    if (!cell) {
        ...
        UITextField* textField = [[UITextField alloc]
            initWithFrame:CGRectMake(10, 0, cell.bounds.size.width - 20,
        ...
        // Numerical keyboards have no return button, we need to add one
        // manually.
...
- (void)textFieldDidEndEditing:(UITextField*)textField {
    ...
    if (textField.keyboardType == UIKeyboardTypeNumberPad) {
        if (textField.text.length != 0) {
            NSNumber* bitrateNumber =
                [NSNumber numberWithInteger:textField.text.intValue];
